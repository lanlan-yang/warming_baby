...
LLM 使用建议：
- 用户想知道事实/知识/最新信息、只关心答案 → 调用本工具
- 用户在浏览器会话中（已用 browser_* 打开网页）的搜索意图 → 优先浏览器工具，不用本工具
"""
import json
...
class WebSearchTool(AgentTool):
    ...
    name: str = "websearch"
    description: str = (
        "联网搜索工具，返回搜索结果摘要供回答使用（结果同时自动在看板窗口展示）。"
        "适合：用户想知道某个事实/知识/最新信息（新闻、版本、价格、攻略等），"
        "只关心答案、不关心浏览过程时调用。"
        "注意：如果本轮对话中你已用浏览器工具打开了网页，"
        "用户的搜索/查找/输入意图优先用浏览器工具在该网页上完成，此时不要调用本工具。"
        "参数：query(搜索关键词，必填)。"
    )
